#ifndef PLAS_GPU_SCAN_KERNEL_H_
#define PLAS_GPU_SCAN_KERNEL_H_
...
void scan_event(input_it input, int count, output_it output, op_t op,
  ...
  typedef typename conditional_typedef_t<
      launch_arg_t, launch_params_t<128, 11> >::type_t launch_t;
  ...
  if (num_ctas > 8) {
    ...
    // Upsweep. Reduce each tile to a scalar and store to partials.
    auto upsweep_k = [=] PLAS_DEVICE(int tid, int cta) {
      ...
      type_t all_reduce =
          reduce_t().reduce(tid, scalar, shared.reduce, tile.count(), op);

      // Store the final reduction to the partials.
      if (!tid) partials_data[cta] = all_reduce;
    };
    cta_transform<launch_t>(upsweep_k, count, context);

    // Spine phase. Recursively call scan on the CTA partials.

    scan_event<scan_type_exc>(partials_data, num_ctas, partials_data, op,
                              reduction, context, event);
    ...
    if (event) cudaEventRecord(event, context.stream());

    // Downsweep phase. Perform an intra-tile scan and add the scan of the
    // partials as carry-in.

    auto downsweep_k = [=] PLAS_DEVICE(int tid, int cta) {
      typedef typename launch_t::sm_ptx params_t;
      enum {
        nt = params_t::nt,
        vt = params_t::vt,
      ...
      __shared__ union {
        ...
        type_t values[nv];  // NOLINT
      } shared;

      // Load a tile to register in thread order.
      range_t tile = get_tile(cta, nv, count);
      array_t<type_t, vt> x = mem_to_reg_thread<nt, vt>(
          input + tile.begin, tid, tile.count(), shared.values);

      // Scan the array with carry-in from the partials.
      array_t<type_t, vt> y =
          scan_t()
              .scan(tid, x, shared.scan, partials_data[cta], cta > 0,
                    tile.count(), op, type_t(), scan_type)
              .scan;
    ...
    auto spine_k = [=] PLAS_DEVICE(int tid, int cta) {
      enum {
        nt = spine_params_t::nt,
        vt = spine_params_t::vt,
        nv = nt * vt
      };
      typedef cta_scan_t<nt, type_t> scan_t;
      ...
      for (int cur = 0; cur < count; cur += nv) {
        ...
        array_t<type_t, vt> x =
            mem_to_reg_thread<nt, vt>(input + cur, tid, count2, shared.values);

        scan_result_t<type_t, vt> result =
            scan_t().scan(tid, x, shared.scan, carry_in, cur > 0, count2, op,
                          type_t(), scan_type);

        // Store the scanned values back to global memory.
        reg_to_mem_thread<nt, vt>(result.scan, tid, count2, output + cur,
                                  shared.values);

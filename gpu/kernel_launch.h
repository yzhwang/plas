#ifndef PLAS_GPU_KERNEL_LAUNCH_H_
#define PLAS_GPU_KERNEL_LAUNCH_H_
...
void cta_launch(func_t f, int num_ctas, context_t& context, args_t... args) {
  ...
  if (num_ctas)
    launch_box_cta_k<launch_box, func_t> << <grid_dim, cta.nt, 0,
                                             context.stream()>>>
        (f, args...);
}
...
void cta_launch(func_t f, const int* num_tiles, context_t& context,
  ...
  auto k = [=] PLAS_DEVICE(int tid, int cta, args_t... args) {
    int count = *num_tiles;
    while (cta < count) {
      f(tid, cta, args...);
      cta += num_ctas;
...
struct transform_f {
  ...
  PLAS_DEVICE void operator()(int tid, int cta, func_t f, size_t count,
                              args_t... args) {
    ...
    strided_iterate<nt, vt, vt0>([=](int i,
                                     int j) { f(range.begin + j, args...); },
                                 tid, range.count());
  }
};
}  // namespace detail
...
void transform(func_t f, size_t count, context_t& context, args_t... args) {
  cta_transform<launch_t>(detail::transform_f<launch_t>(), count, context, f,
                          count, args...);
}
